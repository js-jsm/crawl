...
var Mecab = require('./mecab-mod-sync');
var mecab = new Mecab();
var fs    = require('fs');
vr lineByLine = require('n-readlines');
...
if(args.length <= 0) {
  console.log('node kousei.js textfiles');
  process.exit();
}
...
function checkSentence(line, items, lineno) {
  ...
  for(var i in items) {
    ...
    // 접속사 출현 확인
    if(it[1] == 'MAJ') {
      if(typeof(conj[word]) == 'undefinded') {
        conj[word] = 1;
      } else {
        conj[word]++;
      }
